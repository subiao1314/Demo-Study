...
var HIGHLIGHT_HIS_COLOR = Color.DarkSeaGreen2;
...
//		rect.attr("stroke", this.getPaint());
	...
	highLightHisActivities: function(activityIdList){
		if (!activityIdList || activityIdList.length<=0) {
			//console.error("Activity " + activityId + " not found");
			return;
		}
		for (var i=0;i<activityIdList.length;i++) {
			var shape = this.g.getById(activityIdList[i]);
			if (!shape) {
				console.error("Activity " + activityIdList[i] + " not found");
				return;
			}
			...
			//shape.attr("stroke-width", THICK_TASK_BORDER_STROKE);
			//shape.attr("stroke", HIGHLIGHT_COLOR);
			shape.attr("fill", HIGHLIGHT_HIS_COLOR);
		}
	},
